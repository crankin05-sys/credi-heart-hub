...
const DocumentationJourney = ({ lead, onBack }: JourneyProps) => {
  ...
  const handleContinue = () => {
    setShowAdvice(true);
    fetchAdvice('documents', { name: lead.name, docTypes: selected });
  };
    ...
    <JourneyShell onBack={onBack}>
      ...
      {!showAdvice ? (
        ...
        <>
          ...
          <AdviceCard advice={advice} loading={loading} error={error} onRetry={() => fetchAdvice('documents', { name: lead.name, docTypes: selected })} loadingText="Building your document checklist..." />
