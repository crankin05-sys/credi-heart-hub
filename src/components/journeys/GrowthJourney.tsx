...
const GrowthJourney = ({ lead, onBack }: JourneyProps) => {
  ...
  const handleContinue = () => {
    setShowAdvice(true);
    fetchAdvice('growth', { name: lead.name, focus, website: lead.website });
  };
    ...
    <JourneyShell onBack={onBack}>
      ...
      {!showAdvice ? (
        ...
        <>
          ...
          <AdviceCard advice={advice} loading={loading} error={error} onRetry={() => fetchAdvice('growth', { name: lead.name, focus, website: lead.website })} loadingText="Building your growth plan..." />
