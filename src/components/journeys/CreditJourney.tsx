...
const CreditJourney = ({ lead, onBack }: JourneyProps) => {
  ...
  const handleContinue = () => {
    setShowAdvice(true);
    fetchAdvice('credit', { name: lead.name, situation });
  };
    ...
    <JourneyShell onBack={onBack}>
      ...
      {!showAdvice ? (
        ...
        <>
          ...
          <AdviceCard advice={advice} loading={loading} error={error} onRetry={() => fetchAdvice('credit', { name: lead.name, situation })} loadingText="Building your credit roadmap..." />
